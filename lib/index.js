      ...
      if (typeof window.orientation === 'undefined' && TargetElements.length > 0) {
        ...
        if (Debuggable) {
          (Proxemics.InstancesInfo = () => {
            ...
            TargetElements[TargetElements.length - 1].insertAdjacentHTML('afterend', `<div class="distance-debug${random}"></div>`);
        ...
        const Calculator = () => {
          Array.prototype.forEach.call(TargetElements, element => {
            ...
            const squareRoot = Math.sqrt((Proxemics.X - elX) ** 2 + (Proxemics.Y - elY) ** 2) - Territory;
            const distance = ~~Math.max(squareRoot, 0);
            const radian = Math.atan2(Proxemics.Y - elY, Proxemics.X - elX);
        ...
        if (TargetElements.length === 0) {
          console.log('Proximics error: There is no selector onject on this document.');
        }

        if (typeof window.orientation !== 'undefined') {
          console.log('Proxemics warn: Only working on mouse device.');
        }
      }
    };
